...
nodeSelection
  ...
  .attr('opacity', d => {
    ...
    return (d.group === themaName || (d.group === 'thema' && d.name === themaName)) ? 1 : 0.2;
  });

  // Highlight outer overlay nodes
  d3.selectAll('.outer-dots circle')
    .transition().duration(200)
    .attr('opacity', d => d.group === themaName ? 1 : 0.2)

  // Highlight overlay labels
  d3.selectAll('.outer-dots .label text')
    .transition().duration(200)
    .attr('opacity', d => d.group === themaName ? 0.9 : 0.2);


  // Highlight overlay labels
d3.selectAll('.arc-end')
  .transition().duration(200)
  .attr('opacity', d => (d.source.group === themaName || d.target.group === themaName) ? 0.9 : 0.2);
...
d3.selectAll('.arc-group')
  ...
  d3.selectAll('.ring-links line')
    .transition().duration(200)
    .attr('stroke-opacity', d => d.group === themaName ? 1 : 0.1);
