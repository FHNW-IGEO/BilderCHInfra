...
svg.selectAll("path")
    .attr("d", geoPathGenerator); // Redraw map paths
    let sampleData;
    let geomData;
		try {
		...
		const connectors = gLinks.selectAll('line')
			...
			.attr('x1', d => calculateConnectorCoordinates(d, cx, cy, 6).x1)
			.attr('y1', d =>calculateConnectorCoordinates(d, cx, cy, 6).y1) 
		...
		gLinks.selectAll('.connector-hitbox')
			...
			.on('mouseenter', (event, d) => highlightNode(svg, d.id, true, ITEM_OPACITY_HIGH, ITEM_OPACITY_MID, ITEM_OPACITY_LOW))
			.on('mouseleave', (event, d) => highlightNode(svg, d.id, false, ITEM_OPACITY_HIGH, ITEM_OPACITY_MID, ITEM_OPACITY_LOW));
		connectors.lower();
    ...
    .each(function(d) {
        ...
        coordinates.forEach(line => {
            ...
            group.append("path")
                ...
                .attr("stroke", "#2c2c2cff")
                .attr("stroke-width", 1)
                .attr("opacity", ITEM_OPACITY_LOW) // Initially hidden
                .style("pointer-events", "none");
        });
    });
...
arcGroups.on('mouseenter', (event, d) => {
    highlightNode(svg, d.source.id, true, ITEM_OPACITY_HIGH, ITEM_OPACITY_MID, ITEM_OPACITY_LOW);
    highlightNode(svg, d.target.id, true, ITEM_OPACITY_HIGH, ITEM_OPACITY_MID, ITEM_OPACITY_LOW);
    ...
    // Highlight the link geometry
    svg.selectAll(`.link-geom-${d.source.id}-${d.target.id} path`)
        .transition()
        .duration(200)
        .attr("opacity", ITEM_OPACITY_HIGH);
}).on('mouseleave', (event, d) => {
    highlightNode(svg, d.source.id, false, ITEM_OPACITY_HIGH, ITEM_OPACITY_MID, ITEM_OPACITY_LOW);
    highlightNode(svg, d.target.id, false, ITEM_OPACITY_HIGH, ITEM_OPACITY_MID, ITEM_OPACITY_LOW);
    ...
    // Unhighlight the link geometry
    svg.selectAll(`.link-geom-${d.source.id}-${d.target.id} path`)
        .transition()
        .duration(200)
        .attr("opacity", ITEM_OPACITY_LOW);
});
