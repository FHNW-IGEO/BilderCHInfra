(function() {
const DATA_URL = '/BilderCHInfra/visualisation_examples/data/data.json';
...
nodesRaw.forEach(n => {
    ...
    n.geometry = match ? match.geometry : null;
...
gGeomPaths.selectAll("g.geom-group")
  .data(innerNodes.filter(d => d.geometry && d.geometry.type === "MultiLineString"))
  .enter()
  .append("g")
  ...
  .each(function(d) {
    ...
    multilinestring.forEach(line => {
      const pathData = line.map(coord => {
        ...
        const [x, y] = projection([coord[0], coord[1]]); // lon, lat
        return `${x},${y}`;
